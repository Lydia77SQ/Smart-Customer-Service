"""F-008 测试夹具：独立库 + dependency_overrides[get_db]，禁止操作运行时业务库。"""
...
SUGGESTION_TEXT = "请确认是否使用公司门户的 VPN 客户端，并尝试忘记密码后用邮箱验证码重置。"
CLOSED_REPLY_BODY = {"code": "CONFLICT", "message": "已完结，不能再发送", "data": None}
NEED_ACCEPT_BODY = {"code": "CONFLICT", "message": "请先接入后再回复", "data": None}
...
MESSAGE_KEYS = {"id", "sender_type", "content", "created_at"}
DETAIL_KEYS = {
    "id",
    "title",
    "status",
...
async def add_suggestion(
    ...
    *,
    ticket_id: int,
    content: str,
    result_type: str = "generated_answer",
) -> int:
    async with isolated_session_maker() as db:
        row = Suggestion(
            ticket_id=ticket_id,
